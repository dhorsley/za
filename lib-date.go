...
func buildDateLib() {
	...
	stdlib["time_diff"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		if ok, err := expect_args("time_diff", args, 1, "2", "int", "int"); !ok {
			return nil, err
		}
	...
	slhelp["format_date"] = LibHelp{in: "[epoch]", out: "string", action: "Format date as string from current time or epoch."}
	stdlib["format_date"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		if ok, err := expect_args("format_date", args, 2,
			"1", "int",
			"0"); !ok {
			return nil, err
		}
	...
	slhelp["format_time"] = LibHelp{in: "[epoch]", out: "string", action: "Format time as string from current time or epoch."}
	stdlib["format_time"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		if ok, err := expect_args("format_time", args, 2,
			"1", "int",
			"0"); !ok {
			return nil, err
		}
