	...
	defer func() {
		if r := recover(); r != nil {
			...
			if currentErrorStyle == ERROR_STYLE_EXCEPTION || currentErrorStyle == ERROR_STYLE_MIXED {
				...
				stackTraceCopy := captureStackTraceAtThrow(p.line, calltable[p.fs].fs)
...
binloop1:
	...
	*/
	...
	exceptionPtr := atomic.LoadPointer(&calltable[p.fs].activeException)

	if exceptionPtr != nil {
		// There's an active exception - throw it so it can be caught by try/catch blocks
