...
func wrappedEval(fs uint64, expr ExpressionCarton, interpol bool) (result ExpressionCarton, ef bool) {
	...
	if pos != -1 {
		...
		if epos != -1 {
			...
			case string:
				vsetElement(fs, expr.assignVar[:pos], element, expr.result)
			case int:
				if element < 0 {
					pf("**debug** negative array element found (%v,%v,%v)\n", fs, expr.assignVar[:pos], element)
					expr.evalError = true
					return expr, true
				}
				vsetElement(fs, expr.assignVar[:pos], sf("%v", element), expr.result)
			}
		} else {
			bnest++
		}
