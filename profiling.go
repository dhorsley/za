...
type speedscopeFile struct {
	...
	Profiles []speedscopeEvented `json:"profiles"`
}

var (
	profileEventsMu   sync.Mutex
	profileTracks     = make(map[uint64][]profileEvent)
	profileFrames     = make(map[string]int)
	profileOpenFrames = make(map[uint64][]int)
	profileStart      = time.Now()
)

func recordProfileEvent(open bool, name string, at time.Time) {
	if !enableProfileEvents || name == "" {
		return
	}
	profileEventsMu.Lock()
	gid := getGoroutineID()
	frame, ok := profileFrames[name]
	if !ok {
		frame = len(profileFrames)
		profileFrames[name] = frame
	}
	kind := "C"
	if open {
		kind = "O"
		profileOpenFrames[gid] = append(profileOpenFrames[gid], frame)
	} else if stack := profileOpenFrames[gid]; len(stack) > 0 {
		frame = stack[len(stack)-1]
		profileOpenFrames[gid] = stack[:len(stack)-1]
	}
	profileTracks[gid] = append(profileTracks[gid], profileEvent{Type: kind, Frame: frame, At: at.Sub(profileStart).Nanoseconds()})
	profileEventsMu.Unlock()
}

func recordProfileInterval(name string, duration time.Duration) {
	if !enableProfileEvents || name == "" || duration <= 0 {
		return
	}
	end := time.Now()
	recordProfileEvent(true, name, end.Add(-duration))
	recordProfileEvent(false, name, end)
}

func dumpProfileEvents(path string) error {
	profileEventsMu.Lock()
	defer profileEventsMu.Unlock()
	var out speedscopeFile
	out.Schema = "https://www.speedscope.app/file-format-schema.json"
	out.Name, out.ActiveProfile = "Za profile", 0
	for name, frame := range profileFrames {
		for len(out.Shared.Frames) <= frame {
			out.Shared.Frames = append(out.Shared.Frames, struct {
				Name string `json:"name"`
			}{})
		}
		out.Shared.Frames[frame].Name = name
	}
	end := time.Since(profileStart).Nanoseconds()
	ids := make([]uint64, 0, len(profileTracks))
	for id := range profileTracks {
		ids = append(ids, id)
	}
	sort.Slice(ids, func(i, j int) bool { return ids[i] < ids[j] })
	for _, id := range ids {
		events := append([]profileEvent(nil), profileTracks[id]...)
		sort.SliceStable(events, func(i, j int) bool {
			if events[i].At == events[j].At {
				return events[i].Type == "C" && events[j].Type == "O"
			}
			return events[i].At < events[j].At
		})
		out.Profiles = append(out.Profiles, speedscopeEvented{Type: "evented", Name: sf("goroutine %d", id), Unit: "nanoseconds", EndValue: end, Events: events})
	}
	if len(out.Profiles) == 0 {
		out.Profiles = append(out.Profiles, speedscopeEvented{Type: "evented", Name: "Za", Unit: "nanoseconds", EndValue: end, Events: []profileEvent{}})
	}
