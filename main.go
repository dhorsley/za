...
// Global: sanitisation control
var (
	permit_sanitisation bool         = true // default: true, allow sanitisation control
	sanitisationMutex   sync.RWMutex        // protect sanitisation flag
)
...
func main() {
	...
	var a_profile_events = flag.Bool("PP", false, "write evented Speedscope profile to za-profile.json")
	...
	if *a_parse_timing || *a_parse_timing_verbose {
		...
		success := runParseTiming(exec_file_name, level, *a_parse_timing && !*a_parse_timing_verbose)
		if success {
			os.Exit(0)
		} else {
			os.Exit(1)
...
`
	...
	*/
	...
	if metricsPort > 0 {
		...
		metricsBindAddr := os.Getenv("ZA_PROMETHEUS_BIND")
		startMetricsServer(metricsPort, metricsAllowCIDRs, metricsBindAddr)
	}
	...
	if enableProfileEvents {
		if err := dumpProfileEvents("za-profile.json"); err != nil {
			fmt.Fprintf(os.Stderr, "failed to write Speedscope profile: %v\n", err)
		}
	}
