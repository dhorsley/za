    ...
    for i < len(input) {
        ...
        if !inString {
            if ch == '"' || ch == '\'' || ch == '`' {
                ...
                braceLevel--
            } else if ch == ',' && level == 1 && braceLevel == 0 {
                args = append(args, str.TrimSpace(current))
                current = ""
                i++
                continue
        ...
        for i < len(input) {
            ...
            if !inString && ch == '#' {
                ...
                if j < len(input) && (input[j] == '_' || (input[j] >= 'a' && input[j] <= 'z') || (input[j] >= 'A' && input[j] <= 'Z')) {
                    // start of identifier
                    start := j
                    for j < len(input) && (input[j] == '_' || (input[j] >= 'a' && input[j] <= 'z') || (input[j] >= 'A' && input[j] <= 'Z') || (input[j] >= '0' && input[j] <= '9') || input[j] == '!') {
                        j++
                    }
                    nameWithSuffix := input[start:j]
                    toString := false
                    name := nameWithSuffix
...
// isInt checks if string represents an integer
...
// isFloat checks if string represents a float
func isFloat(s string) bool {
    // Simple check for digits with optional decimal
    hasDot := false
    if s == "" {
        return false
    }
...
// isArrayLiteral checks if string looks like an array literal
func isArrayLiteral(s string) bool {
    return len(s) >= 2 && s[0] == '[' && s[len(s)-1] == ']'
}

// formatArg formats an arg for varargs array, preserving types where possible
func formatArg(arg string) string {
    unq := unquote(arg)
    if isInt(unq) || isFloat(unq) {
        return unq
    }
    if unq == "true" || unq == "false" {
        return unq
    }
    if isArrayLiteral(unq) {
        return unq
    }
    return fmt.Sprintf("%q", unq)
}
...
func ihelpPlugin(args ...string) {
    ...
    if args[0] == "find" {
        if len(args) < 2 {
            pf("Usage: help plugin find <function_name>\n")
            return
        }
...
func help_plugin(ns string, libraryName string) {
    if libraryName == "" {
        ...
        pf("[#4]Loaded C libraries:[#-]\n")
        for libName, lib := range loadedCLibraries {
            symbolCount := len(lib.Symbols)
            pf(sf("  [#6]%s[#-] -> [#2]%s[#-] [#dim](%d symbols)[#-]\n", libName, lib.Name, symbolCount))
        }
    ...
    if !exists {
        pf(sf("[#1]C library '%s' not found.[#-]\n", libraryName))
        return
    }

    pf(sf("[#4]C Library: %s[#-]\n", libraryName))
    pf(sf("Symbols found: [#6]%d[#-]\n", len(lib.Symbols)))

    if len(lib.Symbols) == 0 {
        pf("[#dim]No symbols discovered in this library.[#-]\n")
        return
    }
...
func help_plugin_find(ns string, functionName string) {
    ...
    pf(sf("[#2]Found in %d librar%s:[#-]\n", len(foundIn), func() string {
        if len(foundIn) == 1 {
            return "y"
        }
        return "ies"
    ...
    if err == nil && sig != nil {
        ...
        for _, libName := range foundIn {
            pf(sf("  [#dim]LIB %s::%s(<params>) -> <return_type>[#-]\n", libName, actualFunctionName))
        }
        ...
        pf("  [#dim]LIB c::malloc(size:int) -> pointer[#-]\n")
