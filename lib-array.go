...
func buildArrayLib() {
	...
	slhelp["where"] = LibHelp{in: "condition_string,array1,array2?", out: "array", action: "Select elements from array based on condition string."}
	stdlib["where"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		// Validate arguments: where(condition_string, array) or where(condition_string, array1, array2) or where(array, condition_string)
		if ok, err := expect_args("where", args, 3, "2", "string", "any", "3", "string", "any", "any", "2", "any", "string"); !ok {
			return nil, err
		}
		...
		// Detect pattern: where(condition_string, array1, array2?) vs where(array1, condition_string)
		if _, isString := args[0].(string); isString {
			// New pattern: where(condition_string, array1, array2?)
			conditionStr = args[0].(string)
			array1 = args[1]
		} else {
			// Backward compatibility: where(array1, condition_string)
			array1 = args[0]
			conditionStr = args[1].(string)
		}

		// Use condition string parsing logic
		var reduceparser *leparser
		reduceparser = &leparser{}
		reduceparser.ident = ident
		reduceparser.fs = evalfs
		...
		// Handle scalar case - create default array of reasonable length
		var arrayValue reflect.Value
		var sliceLen int

		if reflect.TypeOf(array1).Kind() == reflect.Slice {
			// Array case
			arrayValue = reflect.ValueOf(array1)
			sliceLen = arrayValue.Len()
		} else {
			// Scalar case - create default array of length 10
			sliceLen = 10
			arrayValue = reflect.MakeSlice(reflect.TypeOf([]any{}), sliceLen, sliceLen)
			for i := 0; i < sliceLen; i++ {
				arrayValue.Index(i).Set(reflect.ValueOf(i))
			}
		}
		...
		for i := 0; i < sliceLen; i++ {
			...
			if reflect.TypeOf(array1).Kind() == reflect.Slice {
				...
				element = i // Use index as element for scalar case
			}
			...
			// If condition is true, add corresponding element to result
			if boolVal, ok := val.(bool); ok && boolVal {
				if len(args) == 3 {
					// Return element from second argument if available
					arg2Value := reflect.ValueOf(args[2])
					if arg2Value.Kind() == reflect.Slice && i < arg2Value.Len() {
						result = append(result, arg2Value.Index(i).Interface())
					} else {
						// Use scalar value
						result = append(result, args[2])
					}
				} else {
					// Return element from first argument
					if reflect.TypeOf(array1).Kind() == reflect.Slice {
