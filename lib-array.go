...
// to2DArray converts any array to a 2D representation
func to2DArray(arr any) [][]any {
	if !isSlice(arr) {
		return [][]any{{arr}}
	}
	...
	if isSlice(firstElem) {
		// Already 2D, convert each row
		result := make([][]any, length)
		for i := 0; i < length; i++ {
			row := value.Index(i).Interface()
			if isSlice(row) {
				rowValue := reflect.ValueOf(row)
				rowLength := rowValue.Len()
				rowSlice := make([]any, rowLength)
				for j := 0; j < rowLength; j++ {
					rowSlice[j] = rowValue.Index(j).Interface()
				}
				result[i] = rowSlice
			} else {
				result[i] = []any{row}
		...
		result := make([][]any, length)
		for i := 0; i < length; i++ {
			result[i] = []any{value.Index(i).Interface()}
		}
		return result
	}
}
...
func buildArrayLib() {
	...
	stdlib["mean"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		if len(args) == 0 || len(args) > 3 {
			return nil, errors.New("mean: requires 1-3 arguments (array, axis?, keepdims?)")
		}

		array := args[0]
		if !isSlice(array) {
			return nil, errors.New("mean: first argument must be an array")
		}

		// Default behavior (backward compatibility)
		if len(args) == 1 {
			return avg_multi(array), nil
		}
		...
		result, err := applyAlongAxisList(array, axis, func(slice []any) any {
			return avg_multi(slice)
		})
		if err != nil {
			return nil, err
		}
	...
	slhelp["std"] = LibHelp{in: "array,axis?,keepdims?", out: "number|array", action: "Calculate the standard deviation of values. axis: -1/None=flatten, 0=first dim, 1=second dim. keepdims: preserve dimensions."}
	stdlib["std"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		if len(args) == 0 || len(args) > 3 {
			return nil, errors.New("std: requires 1-3 arguments (array, axis?, keepdims?)")
		}

		array := args[0]
		if !isSlice(array) {
			return nil, errors.New("std: first argument must be an array")
		}
	...
	// variance() - Variance
	slhelp["variance"] = LibHelp{in: "array,axis?,keepdims?", out: "number|array", action: "Calculate the variance of values. axis: -1/None=flatten, 0=first dim, 1=second dim. keepdims: preserve dimensions."}
	stdlib["variance"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		if len(args) == 0 || len(args) > 3 {
			return nil, errors.New("variance: requires 1-3 arguments (array, axis?, keepdims?)")
		}

		array := args[0]
		if !isSlice(array) {
			return nil, errors.New("variance: first argument must be an array")
		}
	...
	slhelp["find"] = LibHelp{in: "array,condition/value", out: "array", action: "Find indices of non-zero elements, elements matching value, or elements matching condition string."}
	stdlib["find"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		// Validate arguments: find(array) or find(array, condition_string) or find(array, value)
		if ok, err := expect_args("find", args, 3, "1", "any", "2", "any", "string", "2", "any", "any"); !ok {
			return nil, err
		}
		...
		if len(args) == 1 {
			...
			// Check if second argument is a condition string
			if conditionStr, isCondition := args[1].(string); isCondition {
				// find(array, condition_string) - return indices where condition true
				// Use the same parser setup as list_filter
				...
				for i, element := range flat {
					// Use common condition string processing
					newCondition, err := processConditionString(conditionStr, element, i)
					if err != nil {
						return nil, err
					}

					// Evaluate the processed condition
					val, err := ev(reduceparser, evalfs, newCondition)
					if err != nil {
						return nil, err
					}

					// If condition is true, add index to result
	...
	// where() - Conditional selection using condition strings
	slhelp["where"] = LibHelp{in: "input_array,condition[,true_values][,false_values]", out: "array", action: "Select elements from array based on condition. Supports chaining and scalar broadcasting."}
	stdlib["where"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		if ok, err := expect_args("where", args, 4,
			"4", "any", "string", "any", "any", "any",
			"3", "any", "string", "any",
			"2", "any", "string"); !ok {
			return nil, err
		}

		var inputArray, condition, trueValues, falseValues any

		// Parse arguments based on count
		switch len(args) {
		case 2:
			// where(input_array, condition) - filtering
			inputArray = args[0]
			condition = args[1]
			trueValues = nil
			falseValues = nil
		case 3:
			// where(input_array, condition, false_values) - conditional selection with default false values
			inputArray = args[0]
			condition = args[1]
			trueValues = nil
			falseValues = args[2]
		case 4:
			// where(input_array, condition, true_values, false_values) - full NumPy-style conditional selection
			inputArray = args[0]
			condition = args[1]
			trueValues = args[2]
			falseValues = args[3]
		default:
			return nil, errors.New("where: invalid number of arguments")
		}

		// Validate that inputArray is actually an array
		if reflect.TypeOf(inputArray).Kind() != reflect.Slice {
		...
		arrayValue := reflect.ValueOf(inputArray)
		sliceLen := arrayValue.Len()
		var result []any
		
		// For 4-argument case, validate and set up arrays
		if len(args) == 4 {
			trueValues = args[2]
			falseValues = args[3]
			
			// Validate array length compatibility
			trueIsSlice := reflect.TypeOf(trueValues).Kind() == reflect.Slice
			falseIsSlice := reflect.TypeOf(falseValues).Kind() == reflect.Slice
			
			if trueIsSlice && falseIsSlice {
				trueArray := reflect.ValueOf(trueValues)
				falseArray := reflect.ValueOf(falseValues)
				
				if trueArray.Len() != falseArray.Len() {
					return nil, errors.New("where: true_values and false_values arrays must have the same length")
				}
			}
			
			// Limit iteration to minimum length of all arrays
			trueLen := sliceLen
			falseLen := sliceLen
			
			if trueIsSlice {
				trueLen = reflect.ValueOf(trueValues).Len()
			}
			if falseIsSlice {
				falseLen = reflect.ValueOf(falseValues).Len()
			}
			
			// Use minimum length to prevent out-of-bounds access
			sliceLen = trueLen
			if falseLen < sliceLen {
				sliceLen = falseLen
			}
		} else if len(args) == 3 {
			falseValues = args[2]
		...
		for i := 0; i < sliceLen; i++ {
			...
			// Evaluate condition for this element
			var conditionResult any

			switch cond := condition.(type) {
			case string:
				// String condition - process and evaluate
				newCondition, err := processConditionString(cond, element, i)
				if err != nil {
					return nil, err
				}
				conditionResult, err = ev(reduceparser, evalfs, newCondition)
				if err != nil {
					continue // Skip on error
				}
			case []bool:
				// Boolean array condition - use corresponding element
				if i < len(cond) {
					conditionResult = cond[i]
				} else {
					continue // Skip if condition array is shorter
				}
			default:
				return nil, errors.New("where: condition must be string or boolean array")
			}

			// Convert condition result to boolean
			boolVal, ok := conditionResult.(bool)
			if !ok {
				continue // Skip if condition doesn't evaluate to boolean
			}

			// Handle result selection based on argument count
			switch len(args) {
			case 2:
				// Filtering: only include elements where condition is true
				if boolVal {
					result = append(result, element)
				}
			case 3:
				// Conditional selection with default false values
				if boolVal {
					// True case: use element from input array
					result = append(result, element)
				} else {
					// False case: use element from falseValues (with scalar broadcasting)
					if reflect.TypeOf(falseValues).Kind() == reflect.Slice {
						falseArray := reflect.ValueOf(falseValues)
						if i < falseArray.Len() {
							result = append(result, falseArray.Index(i).Interface())
						} else {
							result = append(result, falseValues) // Fallback to scalar
						}
					} else {
						// Scalar broadcasting
			...
			case 4:
				// Full NumPy-style conditional selection
				if boolVal {
					// True case: use element from trueValues (with scalar broadcasting)
					if reflect.TypeOf(trueValues).Kind() == reflect.Slice {
						trueArray := reflect.ValueOf(trueValues)
						result = append(result, trueArray.Index(i).Interface())
					} else {
						// Scalar broadcasting
						result = append(result, trueValues)
					}
				} else {
					// False case: use element from falseValues (with scalar broadcasting)
					if reflect.TypeOf(falseValues).Kind() == reflect.Slice {
	...
	slhelp["prod"] = LibHelp{in: "array,axis?,keepdims?", out: "number|array", action: "Calculate the product of values. axis: -1/None=flatten, 0=first dim, 1=second dim. keepdims: preserve dimensions."}
	stdlib["prod"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		if len(args) == 0 || len(args) > 3 {
			return nil, errors.New("prod: requires 1-3 arguments (array, axis?, keepdims?)")
		}

		array := args[0]
		if !isSlice(array) {
			return nil, errors.New("prod: first argument must be an array")
		}

		// Default behavior (backward compatibility)
		if len(args) == 1 {
			return prod_multi(array), nil
		}
	...
	slhelp["stack"] = LibHelp{in: "arrays...,axis?", out: "array", action: "Stack arrays along a new axis. Axis 0 (default) stacks vertically, axis 1 horizontally."}
	stdlib["stack"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		...
		// Parse axis parameter (optional, default 0)
		axis := 0
		arraysStart := 0
		if len(args) > 1 {
			// Check if last argument is axis (number) - support both int and float64
			if lastArg, isFloat := args[len(args)-1].(float64); isFloat {
				axis = int(lastArg)
				arraysStart = len(args) - 1
			} else if lastArg, isInt := args[len(args)-1].(int); isInt {
				axis = lastArg
				arraysStart = len(args) - 1
			}
		}
		...
		// Validate all array arguments are actually arrays
		for i := 0; i < arraysStart; i++ {
			if !isSlice(args[i]) {
				return nil, fmt.Errorf("stack: argument %d must be an array", i+1)
			}
		}

		// Handle different axis stacking
		if axis == 0 {
			// Vertical stacking (default behavior) - create 2D array where each input becomes a row
			flattened := make([][]any, arraysStart)
			for i := 0; i < arraysStart; i++ {
				flattened[i] = flattenSlice(args[i])
			}

			// Check if all arrays have same length for consistent stacking
			if len(flattened) > 1 {
				firstLen := len(flattened[0])
				for i := 1; i < len(flattened); i++ {
					if len(flattened[i]) != firstLen {
						return nil, errors.New("stack: axis 0 requires all arrays to have same length")
					}
				}
			}
			...
			// Horizontal stacking - create 2D array where each input becomes a column
			// Convert all arrays to 1D representation
			var flattened [][]any
			maxLen := 0

			for i := 0; i < arraysStart; i++ {
				flat := flattenSlice(args[i])
			...
			for i, flat := range flattened {
				if len(flat) != maxLen {
					return nil, fmt.Errorf("stack: axis 1 requires all arrays to have same length (array %d has %d elements, expected %d)", i+1, len(flat), maxLen)
				}
			}

			// Stack horizontally - each array becomes a column
			result := make([][]any, maxLen)
			for row := 0; row < maxLen; row++ {
				rowSlice := make([]any, arraysStart)
				for col := 0; col < arraysStart; col++ {
					rowSlice[col] = flattened[col][row]
				}
				result[row] = rowSlice
			}
			...
			return nil, fmt.Errorf("stack: axis %d not supported (only 0 and 1 supported)", axis)
		}
	}

	// concatenate() - Join arrays along existing axis
	slhelp["concatenate"] = LibHelp{in: "arrays...,axis?", out: "array", action: "Join arrays along an existing axis. Axis 0 (default) concatenates vertically, axis 1 horizontally."}
	stdlib["concatenate"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
		...
		// Handle different axis concatenation
		if axis == 0 {
			// Vertical concatenation (default behavior) - flatten and append
			var result []any
			for i := 0; i < arraysStart; i++ {
				flat := flattenSlice(args[i])
			...
			// Horizontal concatenation - requires 2D arrays with same number of rows
			// Convert all arrays to 2D representation
			var matrices [][][]any
			maxRows := 0

			for i := 0; i < arraysStart; i++ {
				matrix := to2DArray(args[i])
			...
			for i, matrix := range matrices {
				if len(matrix) != maxRows {
					return nil, fmt.Errorf("concatenate: axis 1 requires all arrays to have same number of rows (array %d has %d rows, expected %d)", i+1, len(matrix), maxRows)
				}
			}
			...
			for row := 0; row < maxRows; row++ {
				...
				for _, matrix := range matrices {
					if row < len(matrix) {
						rowConcat = append(rowConcat, matrix[row]...)
					}
				}
				result[row] = rowConcat
			}

			return result, nil
		} else {
			return nil, fmt.Errorf("concatenate: axis %d not supported (only 0 and 1 supported)", axis)
		}
	}
