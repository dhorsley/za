...
func wlog(s string, va ...interface{}) {
	...
	if log_web {
		...
		if WlogDisplay {
			...
			if throttle {
				web_logger.Printf("// skipped %d repeat messages.\n", lastWlogEvery)
			}
			if throttleEnded {
				web_logger.Printf("// stopped throttling messages.\n")
			}
...
func webRouter(w http.ResponseWriter, r *http.Request) {
	...
	if dlevel > 5 {
		wlog("^ NEW REQUEST : host [%v] path [%v]\n", host, purl)
	}
	...
	if handle == "" {
		port := ""
		if dlevel > 5 {
			wlog("  entered empty handle processing\n")
		}
		// extract port
		reqHostSplitAt := str.IndexByte(host, ':')
		if reqHostSplitAt > -1 {
		...
		if dlevel > 5 {
			wlog("  new handle: %v [with 0.0.0.0:%v]\n", handle, port)
		}
	} else {
		if dlevel > 5 {
			wlog("  using handle: %v [with %v]\n", handle, host)
		}
	}
	...
	for _, rule := range wr_copy {
		...
		case "r": // redirect
			...
			if str.HasPrefix(path, rule.in) {
				...
				if WebSendMetrics {
					fireMetric(host, "r", http.StatusMovedPermanently)
				}
				serviced = true
			}
						...
						case 'e':
							...
							if str.HasPrefix(path, fail_rule.in) {
								if expectedStatusCode == "" || expectedStatusCode == sf("%d", down_code) {
									...
									if WebSendMetrics {
										fireMetric(host, "e", http.StatusTemporaryRedirect)
									}
									serviced = true
									break
								} else {
...
func buildNetLib() {
	...
	stdlib["web_serve_start"] = func(evalfs uint64, args ...interface{}) (ret interface{}, err error) {
		...
		var e error
		var srv http.Server
		var addr string = host + ":" + sf("%v", port)
		srv.Addr = addr
	...
	*/
	...
	stdlib["web_head"] = func(evalfs uint64, args ...interface{}) (ret interface{}, err error) {
		if len(args) != 1 {
			return false, errors.New("Bad args (count) to web_head()")
		}
	...
	stdlib["web_get"] = func(evalfs uint64, args ...interface{}) (ret interface{}, err error) {
		if len(args) != 1 {
			return false, errors.New("Bad args (count) to web_get()")
		}
	...
	stdlib["web_custom"] = func(evalfs uint64, args ...interface{}) (ret interface{}, err error) {
		...
		case 3:
			...
			method_string = args[0].(string)
			loc_string = args[1].(string)
			for k, v := range args[2].(map[string]interface{}) {
				headers[k] = v.(string)
			}
		default:
		...
		if err != nil {
			return []interface{}{"Could not create a new HTTP request in web_custom()", nil, 400}, nil
		}
	...
	stdlib["download"] = func(evalfs uint64, args ...interface{}) (ret interface{}, err error) {
		...
		if len(args) != 1 {
			return "", errors.New("Bad args (count) in download()")
		}
	...
	stdlib["web_download"] = func(evalfs uint64, args ...interface{}) (ret interface{}, err error) {
		if len(args) != 2 {
			return false, errors.New("Bad args (count) to web_download()")
		}
