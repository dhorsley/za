...
func buildNetworkLib() {
    ...
    slhelp["traceroute"] = LibHelp{
        in:     "host, protocol, [port], [max_hops], [timeout_seconds]",
        out:    "slice",
    ...
    stdlib["traceroute"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("traceroute", args, 5,
            "2", "string", "string",
            "3", "string", "string", "int",
        ...
        case "icmp":
            return stdlib["icmp_traceroute"](ns, evalfs, ident, args...)
        case "tcp":
            return stdlib["tcp_traceroute"](ns, evalfs, ident, args...)
        default:
    ...
    stdlib["http_headers"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("http_headers", args, 2,
            "1", "string",
            "2", "string", "map[string]interface{}"); !ok {
            return nil, err
        }
        url := args[0].(string)
        customHeaders := make(map[string]string)
        if len(args) == 2 {
            headersMap := args[1].(map[string]any)
