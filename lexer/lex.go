...
// Binder is a callback for binding identifier names to slots.
type Binder func(fs uint32, name string) uint64

// / get the next available token, as a struct, from a given string and starting position.
func NextToken(input string, fs uint32, curLine *int16, start int, binder Binder) (rv *Result, err error) {
    ...
    for currentChar = thisWordStart + 1; currentChar < lenInput; currentChar += 1 {
        ...
        if tokType == NumericLiteral {
            ...
            if scientific && str.IndexByte(beforeE, input[currentChar]) >= 0 {
                return nil, fmt.Errorf("problem lexing character %c in '%s'", input[currentChar], str.TrimRight(input, "\n"))
            }
    ...
    if tokType != 0 {
        ...
        if tokType == NumericLiteral {
            ...
            for _, ns := range numSeps {
                word = str.Replace(word, string(ns), "", -1)
            }
...
get_nt_eval_point:
    ...
    if tokType == 0 { // assume it was an identifier
        ...
        if len(word) > 0 && binder != nil {
            bin := binder(fs, word)
            carton.Bindpos = bin
            carton.Bound = true
            // fmt.Printf("[#3]lex:bound %s in fs %d with bin %d[#-]\n",word,fs,bin)
        }

        if word == "true" {
            carton.Subtype = SubtypeConst
            carton.TokVal = true
        }
        if word == "false" {
            carton.Subtype = SubtypeConst
...
get_nt_exit_point:
    ...
    if startNextTokenAt >= lenInput {
        eof = true
    }

    rv = &Result{Tok: carton, Pos: startNextTokenAt, Eol: eol, Eof: eof, Borpos: borpos}
    // fmt.Printf("      ... lex exit with : %#v\n",rv)
    return rv, nil
