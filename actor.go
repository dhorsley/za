											...
											if err || !endtryFound {
												...
												if ifs < uint32(len(calltable)) {
													...
													if exceptionPtr != nil {
														...
														retvalues = []any{EXCEPTION_THROWN, excPtr.category, excPtr.message}
													} else {
														retvalues = []any{EXCEPTION_THROWN, "unknown", "unknown error"}
													}
												} else {
													retvalues = []any{EXCEPTION_THROWN, "unknown", "unknown error"}
												}
			...
			if isExceptionActive {
				if isCatchMatched {
					...
					if str.Contains(fs, "try_block_") {
						...
						retvalues = []any{EXCEPTION_THROWN, currentExceptionInfo.category, currentExceptionInfo.message}
						retval_count = 3
						endFunc = true
		...
		default:
			...
			if hasActiveException {
				...
				if !endtryFound || err {
					...
					if excInfo != nil {
						retvalues = []any{EXCEPTION_THROWN, excInfo.category, excInfo.message}
						retval_count = 3
					} else {
						retvalues = []any{EXCEPTION_THROWN, "unknown", "unknown error"}
						retval_count = 3
