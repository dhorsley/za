...
func generateStackTrace() []stackFrame {
	var trace []stackFrame

	// Walk through the error chain to build stack trace (reverse order for proper stack)
	...
	// Debug: Print the error chain length and trace length
	// pf("DEBUG: errorChain length=%d, trace length=%d\n", len(errorChain), len(trace))
...
func stripInstanceNumber(functionName string) string {
...
func formatStackTrace(stackTrace []stackFrame) string {
	...
	for i, frame := range stackTrace {
		result.WriteString(sparkle(fmt.Sprintf("  [#fyellow]%d:[#-] [#fgreen]%s[#-]:[#fred]%d[#-]\n", i+1, frame.function, frame.line)))
	}
...
func captureStackTraceAtThrow(throwLine int16, throwFunction string) []stackFrame {
	...
	cleanFunctionName := stripInstanceNumber(throwFunction)
...
// handleUnhandledException applies the exception strictness policy to an unhandled exception
...
// handleUnhandledExceptionSimple applies the exception strictness policy with basic info
	...
	ondo_reenter: // on..do re-enters here because it creates the new phrase in advance and
		...
		case C_Catch:
			...
			if ifs < uint32(len(calltable)) {
				...
				calllock.RLock()
				isCatchMatched = calltable[ifs].currentCatchMatched
				calllock.RUnlock()
			}
				...
				if inbound.TokenCount >= 4 {
					...
					if we = parser.wrappedEval(ifs, ident, ifs, ident, exprTokens); !we.evalError {
						...
						if matched {
							...
							calllock.Lock()
							calltable[ifs].currentCatchMatched = true
							calllock.Unlock()
							...
							errVar := map[string]any{
								...
								"line":        catchExcPtr.line,
								"function":    catchExcPtr.function,
								"stack_trace": catchExcPtr.stackTrace,
							}

							// Debug: Print what's being stored
							// pf("DEBUG: Storing stack trace with %d frames\n", len(catchExcPtr.stackTrace))
							// for i, frame := range catchExcPtr.stackTrace {
							// 	pf("DEBUG: Frame %d: %s:%d\n", i, frame.function, frame.line)
							// }

							// Debug: Print the exception info details
							// pf("DEBUG: Exception info - category=%v, message=%s, line=%d, function=%s\n",
							// 	catchExcPtr.category, catchExcPtr.message, catchExcPtr.line, catchExcPtr.function)
					...
					// Debug: Print the raw exception pointer info
					// pf("DEBUG: Raw exception info - ptr=%v, catchExcPtr=%v\n", exceptionPtr, catchExcPtr)
					// if catchExcPtr != nil {
					// 	pf("DEBUG: Raw stack trace length=%d\n", len(catchExcPtr.stackTrace))
					// }
		...
		case C_Throw:
			...
			if inbound.TokenCount >= 1 {
				...
				// Capture stack trace at throw time
				var stackTraceCopy []stackFrame
				// Start with the current function where the exception is thrown
				currentFrame := stackFrame{
					function: fs,
					line:     inbound.SourceLine,
					caller:   "",
				}
				stackTraceCopy = append(stackTraceCopy, currentFrame)

				// Capture stack trace directly from call stack at throw time
				// Convert 0-based line number to 1-based for display
				actualLine := inbound.SourceLine + 1
				stackTraceCopy = captureStackTraceAtThrow(actualLine, fs)
				...
				// Debug: Print the stored stack trace
				// pf("DEBUG: Stored stack trace with %d frames\n", len(excInfo.stackTrace))
				atomic.StorePointer(&calltable[ifs].activeException, unsafe.Pointer(excInfo))
				calllock.Lock()
				calltable[ifs].currentCatchMatched = false
				calllock.Unlock()
