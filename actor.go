...
func generateStackTrace() []stackFrame {
	...
	pf("DEBUG: generateStackTrace() called, errorChain length=%d\n", len(errorChain))

	// Walk through the error chain to build stack trace (reverse order for proper stack)
	for i := len(errorChain) - 1; i >= 0; i-- {
		chain := errorChain[i]
		pf("DEBUG: errorChain[%d]: name=%s, line=%d\n", i, chain.name, chain.line)
		frame := stackFrame{
			function: chain.name,
			line:     chain.line,
			...
			if interactive && !parser.hard_fault && !parser.std_call && permit_cmd_fallback {
				...
				if shouldConvertToException {
					...
					if excThrow, ok := r.(ExceptionThrow); ok {
						...
						calllock.Lock()
						calltable[csloc].currentCatchMatched = false
						calllock.Unlock()
