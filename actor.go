...
func formatStackTrace(stackTrace []stackFrame) string {
    ...
    for i, frame := range stackTrace {
        ...
        if frame.filename != "" && frame.namespace != "" {
            result.WriteString(sparkle(fmt.Sprintf("  [#fyellow]%d:[#-] [#fgreen]%s[#-] in [#fblue]%s[#-] at [#fcyan]%s[#-]:[#fred]%d[#-]\n", i+1, frame.function, frame.namespace, frame.filename, frame.line)))
        } else if frame.filename != "" {
            result.WriteString(sparkle(fmt.Sprintf("  [#fyellow]%d:[#-] [#fgreen]%s[#-] at [#fcyan]%s[#-]:[#fred]%d[#-]\n", i+1, frame.function, frame.filename, frame.line)))
        } else if frame.namespace != "" {
            result.WriteString(sparkle(fmt.Sprintf("  [#fyellow]%d:[#-] [#fgreen]%s[#-] in [#fblue]%s[#-]:[#fred]%d[#-]\n", i+1, frame.function, frame.namespace, frame.line)))
        } else {
    ...
    if calltable[csloc].caller != 0 {
        ...
        if enhancedErrorsEnabled {
            ...
            errorChain = append(errorChain, chainInfo{
                ...
                name:       caller_str,
                line:       0, // Will be updated when we have the actual source line
...
tco_reentry:
    ...
    for {
        ...
        // Update the line number in the errorChain if we have entries
        if len(errorChain) > 0 {
            // Update the last entry (current function) with the actual source line
            errorChain[len(errorChain)-1].line = inbound.SourceLine
        }
    ...
    ondo_reenter: // on..do re-enters here because it creates the new phrase in advance and
        ...
        case C_Throw:
            ...
            if inbound.TokenCount >= 1 {
                ...
                stackTraceCopy := generateStackTrace()
