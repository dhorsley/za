...
/** ANTLR grammar for the goat language. */
...
assignment_control
    : assign_rule expression
    | ( zero_rule | inc_rule | dec_rule ) Identifier
    ;
...
miscellaneous_control
    : install_rule expression
    | push_rule ( 'graphite' ) expression expression
    | pause_rule expression
    | help_rule expression?
    ;
  // | trigger_rule ( 'script' ) expression C_If expression C_Is expression
  // | download_rule expression expression //url+dest_location
...
io_control
    : input_rule Identifier ( 'param' | 'optarg' | 'key' ) expression
    | prompt_rule Identifier expression expression expression?   // exp1=prompt string, exp2=timeout, exp3=regex input loop filter
    | (log_rule | print_rule ) expression
    | logging_rule ( W_Off | W_On expression? )
    | cls_rule 
    | at_rule vector_2 
    ;
//    | indent_rule expression 
//    : quiet_rule
//    | loud_rule 
...
procedure_control
    ...
    | showdef_rule expression 
    | return_rule expression? 
    ;
...
selectionStatement
    ...
    | on_rule expresssion 'do' statement
