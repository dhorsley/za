...
// windowsFlock implements file locking using Windows LockFile API
// Equivalent to Unix flock() functionality
func windowsFlock(file *os.File, lockType string) error {
    handle := syscall.Handle(file.Fd())

    switch lockType {
    case "r", "read":
        // Windows doesn't have true shared locks like Unix
        // For compatibility, we'll use exclusive locks for read too
        // This is a limitation of Windows file locking
        return lockFileWindows(handle, 0, 0, 0xFFFFFFFF, 0, false)
    case "w", "write":
        // Exclusive lock (write)
        return lockFileWindows(handle, 0, 0, 0xFFFFFFFF, 0, false)
    case "u", "unlock":
        // Unlock
        return unlockFileWindows(handle, 0, 0, 0xFFFFFFFF, 0)
    default:
        return errors.New("Invalid lock type specifier in flock()")
    }
}

func lockFileWindows(handle syscall.Handle, offsetLow, offsetHigh uint32, nNumberOfBytesToLockLow, nNumberOfBytesToLockHigh uint32, bFailImmediately bool) error {
    var flags uint32
    if bFailImmediately {
        flags = 0x00000001 // LOCKFILE_FAIL_IMMEDIATELY
    }

    // Use LockFileEx for better control
...
func unlockFileWindows(handle syscall.Handle, offsetLow, offsetHigh uint32, nNumberOfBytesToUnlockLow, nNumberOfBytesToUnlockHigh uint32) error {
    kernel32 := syscall.NewLazyDLL("kernel32.dll")
    unlockFile := kernel32.NewProc("UnlockFile")

    r, _, err := unlockFile.Call(
        uintptr(handle),
        uintptr(offsetLow),
        uintptr(offsetHigh),
...
func buildFileLib() {
    ...
    stdlib["fopen"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("fopen", args, 1, "2", "string", "string"); !ok {
            return nil, err
        }
    ...
    stdlib["flock"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        ...
        fh := args[0].(pfile)
        lockType := "w" // default to write lock
        if len(args) > 1 {
            lockType = args[1].(string)
        }

        err = windowsFlock(fh.hnd, lockType)
        if err != nil {
            return true, nil // Return true if lock failed (matching Unix behavior)
        }
        return false, nil // Return false if lock succeeded
    }
    ...
    stdlib["fflush"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("fflush", args, 1, "1", "main.pfile"); !ok {
            return nil, err
        }
        fw := args[0].(pfile)
    ...
    stdlib["ftell"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("ftell", args, 1, "1", "main.pfile"); !ok {
            return nil, err
        }
        fw := args[0].(pfile)
    ...
    stdlib["fread"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("fread", args, 1, "2", "main.pfile", "string"); !ok {
            return nil, err
        }
    ...
    stdlib["feof"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("feof", args, 1, "1", "main.pfile"); !ok {
            return nil, err
        }
        fw := args[0].(pfile)
    ...
    stdlib["fwrite"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("fwrite", args, 1, "2", "main.pfile", "string"); !ok {
            return nil, err
        }
        fw := args[0].(pfile)
    ...
    stdlib["fclose"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("fclose", args, 1, "1", "main.pfile"); !ok {
            return nil, err
        }
        fw := args[0].(pfile)
        fw.hnd.Sync()
    ...
    stdlib["file_mode"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("file_mode", args, 1, "1", "string"); !ok {
            return nil, err
        }
        f, err := os.Lstat(args[0].(string))
        if err == nil {
            return int(f.Mode()), err
        }
        return -1, nil
    }
    ...
    stdlib["file_size"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("file_size", args, 1, "1", "string"); !ok {
            return nil, err
        }
    ...
    stdlib["read_file"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("read_file", args, 1, "1", "string"); !ok {
            return nil, err
        }
    ...
    stdlib["stat"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("stat", args, 1, "1", "string"); !ok {
            return nil, err
        }
    ...
    stdlib["is_file"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("is_file", args, 1, "1", "string"); !ok {
            return nil, err
        }
        f, err := os.Stat(args[0].(string))
        if err == nil {
    ...
    stdlib["is_dir"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("is_dir", args, 1, "1", "string"); !ok {
            return nil, err
        }
        f, err := os.Stat(args[0].(string))
        if err == nil {
    ...
    stdlib["perms"] = func(ns string, evalfs uint32, ident *[]Variable, args ...any) (ret any, err error) {
        if ok, err := expect_args("perms", args, 1, "1", "string"); !ok {
            return nil, err
        }
        f, err := os.Stat(args[0].(string))
        if err == nil {
